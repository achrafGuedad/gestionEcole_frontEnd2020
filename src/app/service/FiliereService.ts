...
export class  FiliereService {
  ...
  getFiliereById(codeFiliere: number) {
    ...
    return  this.httpClient.get<Filiere>(this.uri_api_findFiliere_ById,{headers: new HttpHeaders({'Authorization': this.jwt})});
      ...
      updateFiliere(filiere:Filiere,id){
        if (this.jwt == null) {
          this.jwt = this.auth.loadJWT();
        }
        ...
        let headers=new HttpHeaders();
        headers.append('authorization',this.jwt);
        return this.httpClient.put("http://localhost:8080/Filiere/updateFiliere/"+id,filiere,{headers:new HttpHeaders({'Authorization':this.jwt})})
      }
}
