...
export class AdministrationService {
  ...
  getUserById(id:string) {
    ...
    return this.httpClient.get<User>("http://localhost:8080/Account/getUserById/"+id,
      {headers: new HttpHeaders({'Authorization': this.jwt})});
    //.map((res:Response)=>this.convertResponse(res));
  }
  ...
  updateUser(user:User,idUser:number){
    if (this.jwt == null) {
      ...
      let headers=new HttpHeaders();
    headers.append('authorization',this.jwt);
    return this.httpClient.put("http://localhost:8080/Account/updateUser/"+idUser,user,{headers:new HttpHeaders({'Authorization':this.jwt})})
  }
