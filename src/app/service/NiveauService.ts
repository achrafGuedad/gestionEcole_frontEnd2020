...
export  class NiveauService{
  ...
  private uri_api_updateNiveau:string="http://localhost:8080/Niveau/update/";
  ...
  getNiveauById(id:number){
    ...
    return this.httpClient.get<Niveau>("http://localhost:8080/Niveau/getNiveauById/"+id,
      {headers: new HttpHeaders({'Authorization': this.jwt})});
  ...
  updateNiveau(niveau: Niveau,id:number){
    if (this.jwt == null) {
      this.jwt = this.auth.loadJWT();
    }
    ...
    let headers=new HttpHeaders();
    headers.append('authorization',this.jwt);
    return this.httpClient.put(this.uri_api_updateNiveau+id,niveau,{headers:new HttpHeaders({'Authorization':this.jwt})})
  }
