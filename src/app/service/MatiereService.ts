...
export  class MatiereService{
  ...
  public getMatiereById(idMatiere:number){

    if (this.jwt == null) {
      this.jwt = this.auth.loadJWT();
    }
    
    return this.httpClient.get<Matiere>( "http://localhost:8080/Matiere/getMatiereById/"+idMatiere,{headers: new HttpHeaders({'Authorization': this.jwt})});
...
public updateMatiere(matiere:Matiere,id:number){
  if (this.jwt == null) {
    this.jwt = this.auth.loadJWT();
  }
  ...
  let headers=new HttpHeaders();
  headers.append('authorization',this.jwt);
  return  this.httpClient.put( "http://localhost:8080/Matiere/update/"+id,matiere,{headers: new HttpHeaders({'Authorization': this.jwt})} );
